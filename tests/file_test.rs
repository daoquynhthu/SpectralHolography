use spectral_holography::{
    ish_create_with_password, ish_create_with_salt, ish_decrypt_file, ish_decrypt_file_z1,
    ish_encrypt_file, ish_encrypt_file_z1, ish_extract_salt, ish_free_string, ISHContext,
};
...
fn test_file_encryption_decryption() {
    ...
    let result =
        unsafe { ish_encrypt_file(&ctx as *const _ as *mut _, in_c.as_ptr(), out_c.as_ptr()) };
    assert_eq!(result, 0, "Encryption failed");

    // 4. Decrypt File
    let dec_c = CString::new(decrypted_path).unwrap();
    let result =
        unsafe { ish_decrypt_file(&ctx as *const _ as *mut _, out_c.as_ptr(), dec_c.as_ptr()) };
    ...
    assert_eq!(
        ...
        "Decrypted content does not match plaintext"
    );
...
fn test_compression_ratio() {
    ...
    unsafe {
        ish_encrypt_file(&ctx as *const _ as *mut _, in_c.as_ptr(), out_std.as_ptr());
    }

    // Encrypt Z1
    let out_z1 = CString::new(enc_z1_path).unwrap();
    unsafe {
        ish_encrypt_file_z1(&ctx as *const _ as *mut _, in_c.as_ptr(), out_z1.as_ptr());
    }
    ...
    assert!(
        ratio_std > 7.5 && ratio_std < 8.5,
        "Standard mode should have ~8x expansion"
    );
    // Z1 mode packs 4 bytes into 1 f64 (8 bytes), so expansion is 8/4 = 2x.
    // Allow some margin for header/padding
    assert!(
        ratio_z1 > 1.8 && ratio_z1 < 2.5,
        "Z1 mode should have ~2x expansion"
    );
...
fn test_z1_roundtrip_correctness() {
    ...
    // Encrypt Z1
    unsafe {
        ish_encrypt_file_z1(&ctx as *const _ as *mut _, in_c.as_ptr(), out_c.as_ptr());
    }

    // Decrypt Z1
    let result =
        unsafe { ish_decrypt_file_z1(&ctx as *const _ as *mut _, out_c.as_ptr(), dec_c.as_ptr()) };
...
fn test_integrity_check() {
    ...
    assert_eq!(
        res_tampered, -100,
        "Decryption should fail with integrity error"
    );
...
fn test_file_encryption_decryption_z1() {
    ...
    let result =
        unsafe { ish_encrypt_file_z1(&ctx as *const _ as *mut _, in_c.as_ptr(), out_c.as_ptr()) };
    ...
    assert_eq!(
        decrypted_content, plaintext,
        "Z1 Decrypted content does not match plaintext"
    );
...
fn test_salt_storage_and_recovery() {
    ...
    let enc_result = unsafe { ish_encrypt_file(ctx_ptr, in_c.as_ptr(), out_c.as_ptr()) };
    ...
    assert!(
        !ctx_dec_ptr.is_null(),
        "Context recreation with salt failed"
    );

    let dec_c = CString::new(decrypted_path).unwrap();
    let dec_result = unsafe { ish_decrypt_file(ctx_dec_ptr, out_c.as_ptr(), dec_c.as_ptr()) };
    ...
    assert_eq!(
        decrypted_content, plaintext,
        "Decrypted content mismatch with salt recovery"
    );
