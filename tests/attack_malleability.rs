use spectral_holography::{ish_create_with_password, ish_encrypt, ish_decrypt, ish_destroy};
use std::ffi::CString;
...
fn test_malleability() {
    unsafe {
        ...
        if res == 0 {
            ...
            assert_ne!(plaintext[target_idx], decrypted[target_idx], "Target byte should change if integrity check passed");
