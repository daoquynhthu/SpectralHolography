...
#[inline(always)]
fn find_valid_point_ct(field_a: &SpectralField, rng: &mut ChaCha20Rng) -> (Vector3D, f64, u8) {
...
impl ConditionallySelectable for Vector3D {
    fn conditional_select(a: &Self, b: &Self, choice: Choice) -> Self {
        let x = f64::from_bits(u64::conditional_select(
            &a.x.to_bits(),
            &b.x.to_bits(),
            choice,
        ));
        let y = f64::from_bits(u64::conditional_select(
            &a.y.to_bits(),
            &b.y.to_bits(),
            choice,
        ));
        let z = f64::from_bits(u64::conditional_select(
            &a.z.to_bits(),
            &b.z.to_bits(),
            choice,
        ));
...
pub unsafe extern "C" fn ish_extract_salt(input_path: *const c_char) -> *mut c_char {
    ...
    if reader.read_exact(&mut salt_len_bytes).is_err() {
        return std::ptr::null_mut();
    }
    ...
    if reader.read_exact(&mut salt_bytes).is_err() {
        return std::ptr::null_mut();
    }
...
pub unsafe extern "C" fn ish_create_with_password(password: *const c_char) -> *mut ISHContext {
    ...
    let ctx = Box::new(ISHContext::new(
        x,
        y,
        z,
        ref_salt,
...
#[no_mangle]
pub unsafe extern "C" fn ish_create_with_salt(
    password: *const c_char,
    salt_phc: *const c_char,
) -> *mut ISHContext {
...
#[no_mangle]
pub unsafe extern "C" fn ish_encrypt(
    ctx: *mut ISHContext,
    input: *const u8,
    output: *mut u8,
    len: usize,
...
#[no_mangle]
pub unsafe extern "C" fn ish_decrypt(
    ctx: *mut ISHContext,
    input: *const u8,
    output: *mut u8,
    _len: usize,
...
unsafe fn ish_verify_integrity(file_path: &str, mac_key: &[u8; 32]) -> i32 {
    ...
    if file.read_exact(&mut stored_mac).is_err() {
        return -108;
    }
...
#[no_mangle]
pub unsafe extern "C" fn ish_encrypt_file(
    ctx: *mut ISHContext,
    ...
    let result = std::panic::catch_unwind(|| {
        unsafe {
            ...
            if writer.write_all(&iv.to_le_bytes()).is_err() {
                return -4;
            }
            ...
            if let Some(ref salt) = ctx.kdf_salt {
                ...
                if writer.write_all(&salt_len.to_le_bytes()).is_err() {
                    return -4;
                }
                if writer.write_all(salt_bytes).is_err() {
                    return -4;
                }
            } else {
                let salt_len: u32 = 0;
            ...
            if writer.write_all(&[0u8; 8]).is_err() {
                return -4;
            }
            ...
            loop {
                ...
                // Write results
                if writer.write_all(&delta_results).is_err() {
                    return -7;
                }
            ...
            // 4. Update Count
            if writer.seek(SeekFrom::Start(count_pos)).is_err() {
                return -8;
            }
            if writer.write_all(&valid_blocks.to_le_bytes()).is_err() {
                return -9;
            }
...
#[no_mangle]
pub unsafe extern "C" fn ish_decrypt_file(
    ctx: *mut ISHContext,
    input_path: *const c_char,
    output_path: *const c_char,
) -> i32 {
    ...
    let result = std::panic::catch_unwind(|| {
        unsafe {
            ...
            if reader.read_exact(&mut iv_bytes).is_err() {
                return -4;
            }
            ...
            if salt_len > 0 && reader.seek(SeekFrom::Current(salt_len)).is_err() {
                return -4;
            }
            ...
            loop {
                ...
                if writer.write_all(&decrypted_bytes).is_err() {
                    return -7;
                }
...
#[no_mangle]
pub unsafe extern "C" fn ish_encrypt_file_z1(
    ctx: *mut ISHContext,
    input_path: *const c_char,
    output_path: *const c_char,
) -> i32 {
...
#[no_mangle]
pub unsafe extern "C" fn ish_decrypt_file_z1(
    ctx: *mut ISHContext,
    input_path: *const c_char,
    output_path: *const c_char,
) -> i32 {
